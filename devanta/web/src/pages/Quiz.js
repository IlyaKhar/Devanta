...
export function QuizPage() {
    ...
    useEffect(() => {
        ...
        setIsLoading(true);
        api
            .get(`/quiz/${moduleId}?block=${blockIndex}`)
            .then(({ data }) => {
            if (!cancelled) {
                setQuiz(data);
                setAnswers({});
    ...
    async function submitQuiz() {
        ...
        try {
            const { data } = await api.post(`/quiz/${moduleId}/submit?block=${blockIndex}`, { answers: payload });
            setResult(data);
        }
        finally {
